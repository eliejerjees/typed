...
import { getTopTraits } from "@/lib/scoring";
...
export default function ResultScreen({
  ...
  const topTraits = answers ? getTopTraits(answers) : result.traits;
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex w-full max-w-2xl flex-col items-center gap-8 px-8 py-20">
        ...
        >
          <h3 className="text-center text-lg font-bold tracking-widest text-white/70 uppercase">
            Your Recommendations
          </h3>

          {[
            { title: "Movies", items: result.movies },
            { title: "Artists", items: result.artists },
            { title: "Shows", items: result.shows },
          ].map(({ title, items }) => (
            <div key={title}>
